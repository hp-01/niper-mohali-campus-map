...
import { GoogleMap, useJsApiLoader, Polygon, DrawingManager, InfoWindow, DirectionsRenderer } from '@react-google-maps/api';
...
function App() {
  ...
  // --- NAVIGATION ---: New state for navigation
  ...
  // --- NAVIGATION ---: Get user's current location on load
  useEffect(() => {
  ...
  // --- NAVIGATION ---: New function to calculate and display the route
    ...
    directionsService.route(
      {
        ...
        travelMode: window.google.maps.TravelMode.WALKING, // Perfect for a campus
      },
      (result, status) => {
      ...
      <div className="main-content">
        ...
        <header className="admin-panel">
          ...
          <div className="admin-controls">
            {/* --- NAVIGATION ---: Add Clear Route button */}
        ...
        <main className="map-container">
          <GoogleMap mapContainerStyle={MAP_CONTAINER_STYLE} center={mapCenter} zoom={17} onLoad={onMapLoad} options={{ streetViewControl: false, mapTypeControl: false }}>
            {/* --- NAVIGATION ---: Add the DirectionsRenderer component */}
                ...
                {activePolygonId === polygon.id && (
                  <InfoWindow position={getPolygonCenter(polygon.paths)} onCloseClick={() => setActivePolygonId(null)}>
                    <div>
                      ...
                      {/* --- NAVIGATION ---: Add the Navigate Here button */}
                      {userLocation && (
